...
class CloudflareKVCache implements ServerCache {
  ...
  Uri _buildUrl(String key) {
    return Uri.parse(
        'https://api.cloudflare.com/client/v4/accounts/$_accountId/storage/kv/namespaces/$_namespaceId/values/$key');
  }
  ...
  Future<String?> get(String key) async {
    final response = await _client.get(_buildUrl(key), headers: _headers);
  ...
  Future<void> set(String key, String value, {Duration? expiration}) async {
    final data = {
      "value": value,
      if (expiration != null) "expiration_ttl": expiration.inSeconds,
    };
    ...
    if (response.statusCode != 200) {
      throw Exception('Failed to set value for key: $key');
    }
  }

  @override
  Future<void> remove(String key) async {
    final response = await _client.delete(_buildUrl(key), headers: _headers);

    if (response.statusCode != 204) {
      throw Exception('Failed to remove key: $key');
    }
  }
...
//   bool _isConnected() => _connection != null && !_isShutdown;
...
//   void _setUpConnection(RedisConnection newConnection) {
...
//     connection
//         .connect(redisUri.host, redisUri.hasPort ? redisUri.port : 6379)
//         .then((command) async {
//           _command = command;
...
//     } else {
//       try {
//         value = await _command!.get(key).timeout(cacheOperationTimeout,
//             onTimeout: () async {
...
//     try {
//       await _command!.send_object(["DEL", key]).timeout(cacheOperationTimeout,
//           onTimeout: () async {
